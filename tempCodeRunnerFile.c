...
int calculateProductAfterFirstPositive(int array[], int size) {
    int product = 1;
    int firstPositiveFound = 0;//перший додат.елемент в масиві

    for (int i = 0; i < size; i++) {
        if (array[i] > 0) {
            firstPositiveFound = 1;//перевіряє, чи поточний елемент є додатним
            continue;// перехід до наст. інтегр. циклу
        }

        if (firstPositiveFound) {
            product *= array[i];//виконується множення
        }
    }
